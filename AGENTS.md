...
- `node scripts/docs-check.js` verifies all of this, reading the stages from the table above; the edit hook runs it after every change under `docs/` or to this file, and the `docs-check` skill repairs what it reports. The chain covers `docs/` at the repo root only: a repo split into contexts is out of scope.
