...
| the issue tracker itself to set up or change: a project, a workflow, a custom field, a saved search, a dashboard, an automation rule | the expert skill for whichever tracker `MEMORY.md` names. `jira-expert` is the one installed, because the template ships configured for Jira; another tracker needs its own skill installed and named on this row. `to-tickets` stays how a settled plan becomes issues, and `docs/agents/issue-tracker.md` is this repo's tool list, since the canonical one the skill names does not ship with it |
| a document to publish where the business reads it, or a Confluence space to structure: page hierarchy, templates, macros, permissions, a knowledge-base audit | `confluence-expert`. The document itself still follows the chain in `AGENTS.md` and lives in `docs/`; Confluence is where a copy goes for people who will not open the repo. The skill names a canonical tool list that does not ship with it: the Atlassian server in `.mcp.json` surfaces its own Confluence tools, so read them from your tool list rather than looking for that file |
