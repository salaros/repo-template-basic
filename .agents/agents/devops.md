...
4. **Review.** Run `code-review` against the fixed point, then `ponytail-review` over the same diff. Two passes because they hunt different things: one asks whether it is correct, the other whether it should exist.
...
| infrastructure or config that feels over-built | `ponytail-review` for a diff, `ponytail-audit` for the whole repo |
