...
| `MEMORY.md` | The project facts (name, purpose, requirements location, stack, the frontend framework when the project has browser code, and the Jira project when there is one), one per line. Written by the `project-init` skill; absent until it runs. |
| `TODO.md` | The loose-ends ledger: questions nobody has answered, assumptions taken on trust, and work knowingly left undone, one per line as `- [ ] <tag>: <text> (<source>)`. Written by the `loose-ends` skill and absent until the first entry, since an empty ledger says less than no ledger. An entry is deleted in the commit that resolves it, never ticked, so the file's length stays a signal. Needs no issue tracker. |
...
A tracker is optional: a project can plan entirely in `docs/`, and `MEMORY.md` then records `Jira: none`. `code-review`, `to-tickets` and `triage` are the skills that read the issue-tracker configuration, and each asks for one when it is reached for without it. When there is a tracker the template ships configured for **Jira through the Atlassian MCP server**:
